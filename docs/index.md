---
icon: lucide/user-round
---

# Mahdi Haghverdi

**Systems · DevOps · Kubernetes · Backend · Python**

I build software with Python and explore the systems behind it — from Linux, networking, and infrastructure to observability, automation, backend engineering, and Kubernetes.

## Projects

<div class="grid cards" markdown>

- :material-kubernetes:{ .lg .middle } **Kubernetes & DevOps**
    
    ---
    Learning the main concepts of operators.
        
    :octicons-mark-github-24: **[ConfigMapOperator](https://github.com/mahdihaghverdi/ConfigMapOperator)**

    `Kubernetes` `Operator` `Self-healing`

    > A Kubernetes Operator for managing ConfigMaps with ownership and self-healing behavior.

- :material-memory:{ .lg .middle } **Systems & Internals**
    
    ---
    
    Bachelor's Final Project.
    
    :octicons-mark-github-24: **[pys3fuse](https://github.com/mahdihaghverdi/pys3fuse)**

    `FUSE` `S3` `POSIX` `Python`

    > A FUSE filesystem written in Python for accessing S3-compatible object storage through a POSIX filesystem interface.

- :material-server-network:{ .lg .middle } **Networking & Infrastructure**
    
    ---
    
    Building and exploring the infrastructure around software.

    :octicons-mark-github-24: **[ssh-tunnel-with-docker](https://github.com/mahdihaghverdi/ssh-tunnel-with-docker)**

    `SSH` `Docker` `Networking`

    > An experiment with SSH local port forwarding and Docker.

- :fontawesome-brands-python:{ .lg .middle } **Python & Developer Tools**    
    
    ---
    
    Building tools and exploring Python.

    :octicons-mark-github-24: **[chanager](https://github.com/mahdihaghverdi/chanager)**

    `Python` `Developer Tooling`

    > A Python project exploring developer tooling and application-level abstractions.

</div>

---

## What I'm Exploring

<div class="grid cards" markdown>

- :material-server:{ .lg .middle } **Systems & Linux**

    Operating systems, networking, filesystems, and system internals.

- :material-kubernetes:{ .lg .middle } **Kubernetes & Operators**

    Controllers, reconciliation, CRDs, and operator development.

- :material-cog-outline:{ .lg .middle } **DevOps & Infrastructure**

    Observability, automation, containers, and reliable infrastructure.

- :fontawesome-brands-python:{ .lg .middle } **Python Internals**

    CPython, runtime behavior, memory, and performance.

</div>

---
